...
Times_SD = [0.3, 0.45, 1, 1.5, 3, 4.5, 10, 15, 30, 45, 100, 150, 300, 450, 1000, 1500, 3000, 8000];
Times_LD = [3e-10, 4.5e-10, 1e-9, 1.5e-9, 3e-9, 4.5e-9, 1e-8, 1.5e-8, 3e-8, 4.5e-8, 1e-7, 1.5e-7, 3e-7, 4.5e-7, 1e-6, 1.5e-6];%, 3e-6, 4.5e-6, 1e-5, 1.5e-5, 3e-5, 4.5e-5, 1e-4, 2.5e-4];
Time_Ref_Exc = [3,15];
Time_Ref_Pols = [9e-9, 1.1e-8];
    ...
    for k = 1:size(aug_mat,1)/size(time,2)
        ...
        fID = fopen([dirname '\' fname(k).name(1:size(fname(k).name,2)-3) 'tst'], 'w');   
