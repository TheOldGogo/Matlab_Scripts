...
dirname='C:\Users\Patri\OneDrive\My Documents\University Stuff\Placment Stuff\Photoreduction of CO2 via TTAUC\Spectroscopy files 2\Transient Absorption Spectra\TA Data\20180527';
fnames=dir(dirname);

j=1;
for i=1:length(fnames)
    if strfind(fnames(i).name,'626nJ.txt')
...
for i=1:length(txtfnames)
    ...
    view(2)
    D_shifted(:,:,j)=[data_struc(j).data(:,data_struc(j).time<=3e-9), data_struc(j).smootheddata(:,data_struc(j).time>3e-9)];;
    j=j+1;
