...
dirname='C:\Users\gorenfjf\Documents\MYdocuments\projects\fullerene project\PC71BM_Film'
...
for i=1:length(txtfnames)
    ...
    subplot(3,3,j);
    myplot(data_struc(j),[1 3.2],[-5 5])
    view(2)
    D_shifted(:,:,j)=[data_struc(j).data(:,data_struc(j).time<=1.6), data_struc(j).smootheddata(:,data_struc(j).time>1.6)];  % stored the data in the right order in D_shifted.
