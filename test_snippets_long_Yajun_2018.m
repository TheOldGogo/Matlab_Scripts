...
dirname='C:\Users\DECASTCS\Desktop\DR3 vs T\For Matlab\DR3ICC6 300K Sample 1 Yajun';
fnames=dir(dirname);
...
for i=1:length(txtfnames)
    ...
    data_struc(j)=subbackground_SD2018(data_struc(j),[-1e-7 -1e-8]);    
...
for i=1:length(txtfnames)
    ...
    subplot(3,3,j);
    myplot(data_struc(j),[1.08 3.1],[-5e-9 5e-9])
    view(2)
...
lam_min_index = 1;
lam_max_index = 487; %size(data_struc(1).lam,1);
t_min_index = 12;
t_max_index = 192;%size(master_time,2);
