...
for i=1:length(txtfnames)
    ...
    subplot(4,4,j);
    myplot(data_struc(j),[1.3 2.8],[-5e-9 5e-9])
    view(2)
...
lam_min_index = 1;
lam_max_index = 260;%size(data_struc(1).lam,1);
t_min_index = 12;
t_max_index = 44;%size(master_time,2);
...
hold all
for k=1:size(D,3)
    plot(master_time,D_shifted(120,:,k))
    plot(master_time, data_struc(k).data(120,:))
    plot(master_time, data_struc(k).smootheddata(120,:))
