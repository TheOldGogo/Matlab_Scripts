...
comptador=evalin('base','mcr_str.aux.counter;')+1;
...
%         disp('processing row number: '),disp(n)
   ...
   % set appropriately the zero log values
    ...
    evalin('base','mcr_str.InitEstim.aux.xforward=xforward;');
    assignin('base','efl',efl);
    evalin('base','mcr_str.InitEstim.aux.efl=efl;');   
    assignin('base','ef',ef);
    evalin('base','mcr_str.InitEstim.ef=ef;');   
    evalin('base','clear xforward efl ef');
    
    
    evalin('base','mcr_str.aux.counter=1;');
end
...
%     disp('Press any key to see the combined EFA plot (forward + backward)');
    ...
    evalin('base','mcr_str.InitEstim.aux.xbackward=xbackward;');
    assignin('base','ebl',ebl);
    evalin('base','mcr_str.InitEstim.aux.ebl=ebl;');   
 
    assignin('base','eb',eb);
    evalin('base','mcr_str.InitEstim.eb=eb;');  
    evalin('base','clear xbackward ebl eb');  
    evalin('base','mcr_str.aux.counter=2;');
end
...
if comptador==3; % 
    ...
    xforward=evalin('base','mcr_str.InitEstim.aux.xforward;');
    xbackward=evalin('base','mcr_str.InitEstim.aux.xbackward;');
    efl=evalin('base','mcr_str.InitEstim.aux.efl;');
    ebl=evalin('base','mcr_str.InitEstim.aux.ebl;');
    ...
    evalin('base','mcr_str.aux.counter=3;');
end
...
% 2) rescaling the log of eigenvalues plot
    ...
    evalin('base','mcr_str.aux.efamin=1;');
    
    while(evalin('base','mcr_str.aux.efamin')==1)
         pause(0.3);
    end
    ...
    minvalue=evalin('base','mcr_str.InitEstim.aux.EFAminvalue;');
...
%     disp('Press any key to build the initial estimates from EFA results')
    ...
    evalin('base','mcr_str.aux.counter=4;');
...
if comptador==5; % 
    ...
    xforward=evalin('base','mcr_str.InitEstim.aux.xforward;');
    xbackward=evalin('base','mcr_str.InitEstim.aux.xbackward;');
    ef=evalin('base','mcr_str.InitEstim.ef;');
    eb=evalin('base','mcr_str.InitEstim.eb;');
    
    % 3) plot the arranged conc. profiles for the num. of factors

    nf=evalin('base','mcr_str.CompNumb.nc;');
...
%         disp('Press any key to plot the arranged and cleaned profiles')
    ...
    assignin('base','e',e);
    evalin('base','mcr_str.InitEstim.iniesta=e;');  
    evalin('base','clear e'); 
    
    evalin('base','mcr_str.aux.counter=5;');
    set(handles.push_go,'enable','off');  
...
evalin('base','mcr_str.InitEstim.method=''EFA'';');
close;
...
evalin('base','mcr_str.InitEstim.aux.EFAminvalue=EFAminvalue;');
evalin('base','clear EFAminvalue');

evalin('base','mcr_str.aux.efamin=0;');
