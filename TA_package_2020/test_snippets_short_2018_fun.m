...
%sample='OIDTBR';                                                           %now defined in function call
timeoffset=0;
dirname
%dirname='\\datawaha\ufd\Exchanges\For Julien\Offset paper\data\DR3 project\DR3_OIDTBR_SD\720nm_excitation\'    %now defined in function call   
fnames=dir(dirname)                                                        % dir lists the files in directory -> return a structure with file names and infos

j=1;
for i=1:length(fnames)
    if strfind(fnames(i).name,extension)                                    % look in the list the files with matching names
...
j=1;
%lam=lam_stable;                                                            %now defined at the begining
%t=t_zero;
...
j=1;
%t_raw = 0.8;                                                                  % now defined at the begining
...
%t_max = 8000;                                                              % now defined at the begining
%w_min = 0.82;
...
hold all
for k=1:size(D,3)
    plot(master_time,D_shifted(80,:,k))                                    % this 150 (previously 200) is just a random wavelength
     plot(master_time, data_struc(k).data(80,:))
    plot(master_time, data_struc(k).smootheddata(80,:))
    axis([-5, 5, -Inf, inf]);
end
...
data = aug_mat_SD;
energies = master_spectra_reduced_SD;
times = master_time_reduced_SD;
dirname_out = dirname;
files = ordered_names;
