function out=txt2strucfun(varargin)
...
if nargin==0
    ...
    {  '*.txt','txt-files (*.txt)'; ...
       ...
       '*.*',  'All Files (*.*)'}, ...
       'Select Data Files to Process','C:\Dokumente und Einstellungen\howard\Eigene Dateien\manuscripts\stark effect\IR measurements', ...
       'MultiSelect', 'on');
    ...
    if iscell(filename)
        ...
        out=process(fp, filename);
    end
    else
    fp=varargin{1};
    out=process(fp, 'not used');  
    end
end
